...
**分布式键值与网盘存储** 全栈实现：多副本 **Tablet** 存储节点、按 **row key 范围** 路由的 **Coordinator**、**WAL + Checkpoint** 崩溃恢复，以及 **无状态 Web 层**（网盘、邮件、管理台、负载均衡、可选 SMTP）。

### 课程作业说明

本项目为 **学校分布式系统（或同类）课程的团队大作业**，用于课程要求下的设计、实现与验收。若你正在修读类似课程，请 **遵守本校学术诚信与课程规定**：独立完成作业要求，勿直接抄袭；引用或参考本仓库时请按课程说明标注来源。

对外公开仓库仅作 **学习交流、作品集展示** 用途，与课程评分、教务要求无关的部分以任课教师说明为准。
...
- **课程属性**：见上文「课程作业说明」。  
- **使用范围**：学习交流、个人作品集展示；转载或二次分发请保留作者信息并遵守原课程/学校的相关规定。  
- **免责声明**：代码按「现状」提供，不作商用或生产环境担保。
